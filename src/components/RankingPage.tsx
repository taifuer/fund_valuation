...
export default function RankingPage({
  ...
  const items = useMemo(() => {
    ...
    return allItems
      ...
      .sort((a, b) => {
        if (a.returnPercent == null && b.returnPercent == null) return a.name.localeCompare(b.name);
        if (a.returnPercent == null) return 1;
        if (b.returnPercent == null) return -1;
        return sortDirection === 'desc' ? b.returnPercent - a.returnPercent : a.returnPercent - b.returnPercent;
      });
  }, [category, etfFilter, fundEstimates, marketReturns, marketStates, quotes, range, sortDirection]);
  ...
  return (
    <main className={styles.page}>
      <section className={styles.toolbar}>
        ...
        <div className={`${styles.controlBlock} ${styles.sortControl}`} aria-label="排序方向">
          <div className={styles.segmented}>
            <button
              type="button"
              className={`${styles.segmentButton} ${sortDirection === 'desc' ? styles.segmentButtonActive : ''}`}
            ...
            <button
              type="button"
              className={`${styles.segmentButton} ${sortDirection === 'asc' ? styles.segmentButtonActive : ''}`}
