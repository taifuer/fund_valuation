...
export interface FundBacktestPoint {
  date: string;
  predictedChange: number;
  normalizedChange: number;
  rawFittedChange: number;
  normalizedFittedChange: number;
  fittedChange: number;
  actualChange: number;
  error: number;
  fittedError: number;
...
export interface FundBacktestMetrics {
  mae: number | null;
  rmse: number | null;
...
export interface FundBacktestSummary {
  ...
  recommendedModel: 'raw' | 'linear' | 'normalizedLinear';
  shouldApplyFitted: boolean;
  expectedError: number | null;
  train: {
    raw: FundBacktestMetrics;
    linear: FundBacktestMetrics;
    normalizedLinear: FundBacktestMetrics;
    selected: FundBacktestMetrics;
  };
  validation: {
    raw: FundBacktestMetrics;
    linear: FundBacktestMetrics;
  ...
  raw: FundBacktestMetrics;
  normalized: FundBacktestMetrics;
  fitted: FundBacktestMetrics;
  points: FundBacktestPoint[];
  limitations: string[];
}
