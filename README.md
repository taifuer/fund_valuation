...
```
# 从最新官方净值日收盘累计到当前；本地缺少净值日基准时回退到当前单日涨跌
股票累计涨跌幅 = 当前价 ÷ 净值日收盘价 - 1
汇率累计涨跌幅 = 当前汇率 ÷ 净值日参考汇率 - 1
...
- **历史行情**：指数、资产和 ETF 走势使用日 K 数据；A 股指数、美股指数来自新浪日 K，恒生科技使用腾讯港股指数日线，恒生指数/日经225使用对应期货日 K，台湾加权来自台湾证券交易所（TWSE）官方月度历史（使用无重定向的 `www.twse.com.tw` 域名），韩国 KOSPI 来自 Naver Finance 日线并以东方财富作为兜底，黄金、白银和原油来自新浪外盘期货日 K；比特币暂不展示历史弹窗
...
- **T 日实时估算**：优先按最新官方净值日的持仓收盘价与参考汇率计算累计涨跌，再按披露权重加权；本地缺少净值日基准时才回退到当前单日涨跌。外币持仓按 `人民币口径涨跌 = (1 + 持仓累计涨跌) × (1 + 汇率累计涨跌) - 1` 合并，未映射到行情源的持仓不参与估算。
