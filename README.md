...
| 构建 | Vite 6 |
| 样式 | CSS Modules |
| 数据源 | 新浪财经（行情）+ 东方财富/天天基金（基金净值与持仓）+ TWSE（台湾指数历史）+ Naver Finance（日/韩个股与韩国指数） |
| 后端 | Flask + SQLite |
...
刷新 worker 使用 SQLite 租约避免多实例重复抓取，每分钟检查任务计划：现货开盘时行情每 60 秒更新，期货每 2 分钟更新，仅加密资产交易时每 5 分钟更新，全部闭市后降为 15 分钟；基金净值闭市后降为每小时，历史数据每小时检查一次，最新季度持仓每 24 小时批量检查一次，基金规模与费率资料每周检查一次。worker 每天维护当前持仓日线与 ECB 参考汇率，作为跨多个净值日累计估算的本地基准；每次发布完整行情快照后同步计算并保存基金估值快照。页面请求本身不会补抓这些历史数据。发现新季度持仓时会同步更新对应基金资料，新持仓入库后会自动加入行情快照和估值计算。`FUND_VALUATION_REFRESH_INTERVAL` 用于设置历史维护周期下限，`FUND_VALUATION_SNAPSHOT_RETENTION_DAYS` 用于调整行情快照保留天数。核心看板请求不会因快照过期而同步抓取上游；首次查看尚未入库的自定义基金资料时，资料接口会按需补取一次。内置刷新默认关闭，仅兼容旧部署时可显式设置 `FUND_VALUATION_BACKGROUND_REFRESH=1`。
...
- **日/韩持仓行情**：日股和韩股使用 Naver Finance 公共接口，当前生产服务器可直接访问；日本行情通常延迟约 15 分钟，个股历史单次更新最近 60 个交易日。该接口没有稳定性承诺，上游异常时会保留最近一次有效快照，不会用错误市场代码替代。
