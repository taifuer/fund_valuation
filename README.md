...
追踪全球指数、资产、ETF、代表性科技公司与 QDII 主动基金，覆盖实时概览、经营变化、区间收益、回撤风险和按目标日期对齐的基金估算净值。
...
- **科技公司财报**：克制展示 30 家中国、美国、欧洲和韩国代表性科技公司，覆盖互联网、软件、芯片、智能汽车与动力电池，通过版本化离线数据观察营业收入、营业利润、研发投入和员工人数的年度、半年与季度变化，并可独立切换同比趋势；缺失期间不推算，华为等仅有完整年度披露的公司只展示年度趋势
...
- **收益与风险**：通过“收益”入口切换收益排行和风险指标，保留独立地址，支持按资产类型、区间与表头指标筛选排序
- **基金管理**：默认 18 只 QDII 主动基金，支持按科技、全球成长、先进制造、医疗健康和新兴市场筛选；配置管理令牌后可在当前浏览器添加、删除基金或恢复默认列表
- **双口径估值**：默认基金同时展示最新官方净值、最近待公布净值估算与当前估值日实时参考；未披露仓位按投资方向使用单一宽基或复合代理
- **持仓穿透**：点击基金卡片展开前十大持仓明细，展示实时现价、涨跌幅、权重贡献和未披露仓位代理贡献
...
| 构建 | Vite 6 |
| 样式 | CSS Modules |
| 数据源 | 新浪财经（行情）+ 东方财富/天天基金（基金净值与持仓）+ 腾讯港股 + TWSE + Naver Finance + Binance/Coin Metrics + ECB + 公司官方披露（离线维护） |
| 后端 | Flask + SQLite |
...
Flask Web 请求只读取 SQLite 或持久化上游缓存，不同步等待公开数据源。独立 worker 负责抓取上游、原子发布完整看板快照，并把基金净值、最新持仓和日 K 写入 `data/fund_valuation.db`。浏览器每次轮询都会请求服务器，本地快照仅在接口超时或失败时兜底；上游短暂异常时页面继续使用最近一次有效快照。科技公司经营数据独立维护在前端版本中，不参与运行时抓取和轮询。
...
> **口径说明**：默认基金按投资方向显式配置纳指100、标普500或对应宽基作为未披露部分的代理；汇添富全球医疗（004877）使用全球医疗 70%、恒生生物科技 10% 和债券/存款低波动部分 20% 组成的复合代理。复合代理任一必要历史尚未就绪时暂停生成该基金估值，不使用前十大持仓归一化替代。
> worker 会保存待公布估值和实时参考快照，并在对应官方净值披露后记录误差。校准只使用同一模型、同一季度持仓、同一残差基准及同一有效输入集合的最近 120 个完整快照；历史样本达到最低数量、且按时间切分的验证集 MAE 至少改善 10% 时，才用全部入选样本重新拟合并自动启用线性校准，否则始终使用原始模型。
...
└── src/
    ...
    ├── components/
    │   ├── Header.tsx             # 标题栏（实时时钟）
    │   ├── CompaniesPage.tsx      # 科技公司财报指标与变化趋势
    ...
    ├── data/
    │   └── companyFundamentals.ts # 版本化科技公司经营数据
...
- **实时行情**：证券与期货行情主要来自新浪财经，A 股为交易时段实时，美股为北京时间晚间实时。港股和亚太指数非交易时段显示前收盘价；纳指100、标普500、道琼斯、恒生指数和日经225在现货闭市时可显示对应期货参考行情；资产分组展示黄金、白银、原油和比特币行情，比特币优先使用 Binance `BTCUSDT` UTC 日线的最新价与当日开盘价计算日内涨跌，异常时回退新浪行情
...
- **公司财报数据**：人工核对公司官方财报后，以版本化离线数据随前端发布，不在页面访问或 worker 运行时更新。金额保留原披露币种；季度同比按同财季匹配，半年数据仅在两个季度披露完整时生成；研发投入仅采用公司单列披露值；员工人数按公司实际披露频率展示，统计或合并范围发生显著变化时在图中标注可比边界
...
- **医疗基金复合代理**：汇添富全球医疗（004877）的未披露仓位以 IXJ 和港交所上市的华夏恒生生物科技 ETF（03069）近似对应官方基准中的两类权益资产，债券及存款部分按短期零涨跌处理。代理并非基金实际完整组合，仍会受主动调仓、ETF 跟踪误差、债券波动和估值时点影响。
