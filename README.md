...
Docker Worker 默认每天创建一次经过完整性校验的 SQLite 在线备份，并保留 7 天，文件位于数据卷的 `backups/`。可通过 `FUND_VALUATION_AUTO_BACKUP`、`FUND_VALUATION_BACKUP_INTERVAL_HOURS` 和 `FUND_VALUATION_BACKUP_RETENTION_DAYS` 调整；非 Docker 本地运行默认不自动备份。行情快照、上游响应缓存和原始响应均为可再生成数据，Docker 默认分别保留 7、14、7 天；Worker 会定期清理并执行非阻塞 WAL checkpoint 与 `PRAGMA optimize`，不会自动删除历史净值、日线、持仓或回测数据。
