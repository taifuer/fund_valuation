...
部署后仍建议通过独立脚本预热历史数据，避免首次访问时出现历史走势或区间收益为空。
```
...
- **估值回测**：基金详情中的“回测”基于最新披露/配置的前十大持仓，抓取持仓股票历史日 K 后，对比近 90 个官方净值日涨跌。回测会按时间拆分训练集和验证集，对比原始估算、线性拟合和覆盖率归一化拟合的验证误差；当前拟合结果只用于评估，不直接改写首页实时估值。
...
- **回测原始估算**：对历史净值日使用持仓股票日 K 涨跌计算，`原始估算 = Σ(持仓权重 × 持仓当日涨跌幅)`，并与同日官方净值涨跌对比。
...
- **回测局限**：当前回测使用最新前十大持仓回看历史，尚未按历史季度切换持仓，也未纳入历史汇率、现金仓位、费用和未披露持仓；训练/验证拆分可以降低但不能消除短期样本过拟合风险。
