...
- **main**：轻量版本，主要使用 Vite 服务端代理转发公开接口，适合快速体验和静态前端部署。
- **dev**：后端增强版本，使用 Flask + SQLite 抓取、缓存并持久化历史净值、行情和动态基金持仓，适合长期运行和数据回填。
...
> 生产部署时，需要配置反向代理将 `/api/sina`、`/api/fundnav`、`/api/fundhistory`、`/api/markethistory` 转发至对应上游。可参考 `vite-sina-proxy.ts` 中的代理逻辑。
...
- **历史行情**：指数和资产走势使用日 K 数据；A 股指数、美股指数来自新浪日 K，恒生指数/日经225使用对应期货日 K，黄金、白银和原油来自新浪外盘期货日 K；韩国KOSPI、台湾加权和比特币暂不展示历史弹窗，避免无稳定数据源时显示空图
