...
| 构建 | Vite 6 |
| 样式 | CSS Modules |
| 数据源 | 新浪财经（行情）+ 东方财富/天天基金（基金净值）+ TWSE（台湾指数历史） |
| 后端 | Flask + SQLite |
...
- **历史行情**：指数、资产和 ETF 走势使用日 K 数据；A 股指数、美股指数来自新浪日 K，恒生指数/日经225使用对应期货日 K，台湾加权来自台湾证券交易所（TWSE）官方月度历史（使用无重定向的 `www.twse.com.tw` 域名），黄金、白银和原油来自新浪外盘期货日 K；韩国 KOSPI 和比特币暂不展示历史弹窗
...
- **历史 K 线覆盖**：不同市场历史数据长度不一致。A 股新浪日 K 当前可获取的条数有限，因此 5年/全部区间可能不足完整 5 年；恒生指数和日经225使用对应期货历史，不是现货指数历史。台湾加权使用 TWSE 官方数据；韩国交易所 KOSPI Open API 需要申请认证密钥，当前未接入历史序列。
