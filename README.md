...
worker 按现货 1 分钟、期货 2 分钟、加密资产 5 分钟、闭市 15 分钟的节奏更新行情；开盘前 30 分钟自动提高到每分钟检查。行情循环与最新持仓检查、历史回填、备份和清理并行运行，维护任务不会阻塞行情快照发布。Docker 默认保留 7 天行情快照，用于时效诊断和异常回放。
```
...
> **口径说明**：外层展示的估算涨跌/净值为**覆盖率归一化**后的值。原因是基金仅披露前十大持仓（合计权重通常约 70–75%），
...
刷新 worker 使用 SQLite 租约避免多实例重复抓取，每分钟检查任务计划：现货开盘时行情每 60 秒更新，期货每 2 分钟更新，仅加密资产交易时每 5 分钟更新，全部闭市后降为 15 分钟；基金净值闭市后降为每小时，历史数据每小时检查一次，最新季度持仓每 24 小时批量检查一次。新持仓入库后会自动加入行情快照和估值计算。`FUND_VALUATION_REFRESH_INTERVAL` 用于设置历史维护周期下限，`FUND_VALUATION_SNAPSHOT_RETENTION_DAYS` 用于调整行情快照保留天数。Web 进程不会因快照过期而同步抓取上游；内置刷新默认关闭，仅兼容旧部署时可显式设置 `FUND_VALUATION_BACKGROUND_REFRESH=1`。
...
- **T 日实时估算**：基于基金最新有效季度披露的前十大持仓，先做加权涨跌再按**已覆盖持仓权重归一化**后展示（外币持仓并入兑 CNY 汇率涨跌）；展开「持仓」标签页可查看原始加权涨跌与行情覆盖率。worker 每 24 小时检查最新持仓，只接受正式季末报告日，入库后自动用于估算。无法映射到可用行情代码的持仓会展示但不纳入估算。
...
- **覆盖率归一化**：当只有部分持仓能获取行情时，按已覆盖权重放大，`归一化估算 = 原始估算 / 已覆盖持仓权重`；若覆盖权重不足，则估算的参考价值会下降。
