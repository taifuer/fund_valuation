...
1. 查找可能需要更新的公司：
   ...
   `--offline` 只检查本地披露周期，`--all` 展示全部 SEC 与非美来源结果，`--strict` 在发现报告候选或真实来源故障时返回非零状态。联网模式会并发比对美国 SEC 申报，并探测中国、欧洲和亚太公司的官方披露归档；页面命中预期期间时只标记为“报告候选”，仍需人工打开原始文件核验。官方站点的 `403`、`429` 或超时归为“访问受限”，不等同于来源失效。
