...
- 早期日线补齐每30分钟最多4个串行请求，按可用覆盖优先补足5年；记录来源与进度、价格冲突待复核，不进入页面请求链路
...
概览仅展示指数、资产、ETF 与汇率，通过 `/api/dashboard` 读取大盘快照，不请求基金净值、估值、持仓或历史数据。切回概览保留已加载行情，后台刷新不会把可用卡片替换成骨架屏。
...
基金页首次进入默认筛选主动基金，之后在当前浏览器记住分类选择；链接中的 `strategy=active|index|all` 优先于本地偏好。指数基金默认按官方净值涨跌排序，主动基金首次进入按实时参考排序。收益页保留自身独立筛选，不受基金页偏好影响。
...
- `backend/daily_archive.py`：有界、可恢复的早期日线补齐，保留原始价格与独立校验状态
...
- `/api/dashboard`：市场行情与汇率快照，概览页使用
- `/api/overview`：兼容旧版客户端的概览与基金净值摘要，新版概览不再调用
